...
namespace libdefwallet {
    ...
    std::vector<std::string> create_mnemonic(std::vector<uint8_t> entropy,
    ...
    bool validate_mnemonic(const std::vector<std::string>& words,const dictionary& lexicon) {
    ...
    std::array<uint8_t, 64> decode_mnemonic(const std::vector<std::string>& words) {
        ...
        const auto result = pkcs5_pbkdf2(sentence_chunk.data(), sentence_chunk.size(),salt_chunk.data(), salt_chunk.size(), hash.data(), hash.size(), 2048);
        return hash;
    }
